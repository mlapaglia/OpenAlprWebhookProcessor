...
namespace Tests.Controllers
{
    ...
    public class AlertsControllerTests : TestBase
    {
        ...
        [Test]
        public async Task UpdateAlert_ValidAlert_ReturnsOk()
        {
            // Arrange
            var alertId = Guid.NewGuid();
            var alert = TestDataFactory.CreateTestAlert("UPDATE123", "Updated Description");
            alert.Id = alertId;
            var cancellationToken = GetCancellationToken();

            // Act
            var result = await _controller.UpdateAlert(alertId, alert, cancellationToken);

            // Assert
            AssertOkResult(result);
            await Mediator.Received(1).Send(Arg.Any<UpdateAlertCommand>(), cancellationToken);
        }

        [Test]
        public async Task UpdateAlert_CallsCorrectCommand()
        {
            // Arrange
            var alertId = Guid.NewGuid();
            var alert = TestDataFactory.CreateTestAlert("UPDATE123", "Updated Description");
            ...
            // Act
            await _controller.UpdateAlert(alertId, alert, cancellationToken);

            // Assert
            await Mediator.Received(1).Send(
                Arg.Is<UpdateAlertCommand>(cmd => 
                    cmd.Alert.Id == alertId && 
                    cmd.Alert.PlateNumber == "UPDATE123" && 
                    cmd.Alert.Description == "Updated Description"), 
                cancellationToken);
        }

        [Test]
        public async Task UpdateAlert_MismatchedId_ReturnsBadRequest()
        {
            // Arrange
            var urlId = Guid.NewGuid();
            var bodyId = Guid.NewGuid();
            var alert = TestDataFactory.CreateTestAlert("UPDATE123");
            alert.Id = bodyId;
            var cancellationToken = GetCancellationToken();

            // Act
            var result = await _controller.UpdateAlert(urlId, alert, cancellationToken);

            // Assert
            AssertBadRequestResult(result);
            await Mediator.DidNotReceive().Send(Arg.Any<UpdateAlertCommand>(), cancellationToken);
        }

        [Test]
        public async Task DeleteAlert_ValidId_ReturnsOk()
        {
            // Arrange
            var alertId = Guid.NewGuid();
            var cancellationToken = GetCancellationToken();
            ...
            // Act
            var result = await _controller.DeleteAlert(alertId, cancellationToken);

            // Assert
            AssertOkResult(result);
            await Mediator.Received(1).Send(Arg.Any<DeleteAlertCommand>(), cancellationToken);
        }

        [Test]
        public async Task DeleteAlert_CallsCorrectCommand()
        {
            // Arrange
            var alertId = Guid.NewGuid();
            var cancellationToken = GetCancellationToken();
            ...
            // Act
            await _controller.DeleteAlert(alertId, cancellationToken);

            // Assert
            await Mediator.Received(1).Send(
                Arg.Is<DeleteAlertCommand>(cmd => cmd.Id == alertId), 
                cancellationToken);
        }
