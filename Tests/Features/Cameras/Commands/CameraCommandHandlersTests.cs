...
namespace Tests.Features.Cameras.Commands
{
    ...
    public class CameraCommandHandlersTests : TestBase
    {
        ...
        {
            // Arrange
            var handler = new UpsertCameraCommandHandler(UnitOfWork, _cameraScheduler);
        ...
        public async Task UpsertCameraCommandHandler_CameraWithDayNightEnabled_SchedulesDayNightTask()
        {
            ...
            // Assert
            await _cameraScheduler.Received(1).RescheduleAllCamerasAsync();
        }
        ...
        public async Task UpsertCameraCommandHandler_CameraWithDayNightDisabled_DeletesSunriseSunset()
        {
            ...
            // Assert
            await _cameraScheduler.Received(1).RemoveCameraScheduleAsync(camera.Id);
        }
        ...
        {
            // Arrange
            var handler = new DeleteCameraCommandHandler(UnitOfWork, _cameraScheduler);
        ...
        public async Task DeleteCameraCommandHandler_NonExistentCamera_DoesNothing()
        {
            ...
            // Assert
            await _cameraScheduler.DidNotReceive().RemoveCameraScheduleAsync(Arg.Any<Guid>());
        }
        ...
        {
            // Arrange
            var handler = new TestCameraDayModeCommandHandler(_cameraScheduler);
            ...
            // Assert
            await _cameraScheduler.Received(1).ExecuteDayNightModeAsync(cameraId, SunriseSunset.Sunrise);
        }
        ...
        {
            // Arrange
            var handler = new TestCameraNightModeCommandHandler(_cameraScheduler);
            var cameraId = Guid.NewGuid();
            ...
            // Assert
            await _cameraScheduler.Received(1).ExecuteDayNightModeAsync(cameraId, SunriseSunset.Sunset);
        }
        ...
        {
            // Arrange
            var handler = new TestCameraOverlayCommandHandler(_cameraScheduler);
            var cameraId = Guid.NewGuid();
            ...
            // Assert
            await _cameraScheduler.Received(1).ScheduleOverlayAsync(
        ...
        {
            // Arrange
            var handler = new SetZoomAndFocusCommandHandler(_cameraScheduler);
            var cameraId = Guid.NewGuid();
            ...
            // Assert
            await _cameraScheduler.Received(1).SetZoomAndFocusAsync(cameraId, zoomFocus, cancellationToken);
        }
        ...
        {
            // Arrange
            var handler = new TriggerAutofocusCommandHandler(_cameraScheduler);
            var cameraId = Guid.NewGuid();
            var command = new TriggerAutofocusCommand(cameraId);
            var cancellationToken = CancellationToken.None;

            _cameraScheduler.TriggerAutofocusAsync(cameraId, cancellationToken)
                .Returns(true);
            ...
            // Assert
            result.Should().BeTrue();
            await _cameraScheduler.Received(1).TriggerAutofocusAsync(cameraId, cancellationToken);
        }
