...
namespace Tests.Features.LicensePlates.Queries.SearchLicensePlates
{
    ...
    public class SearchLicensePlatesQueryHandlerTests : TestBase
    {
        ...
        [Test]
        public async Task Handle_IncludeIgnoredPlates_IncludesIgnoredPlates()
        {
            // Arrange
            ...
            {
                PlateNumber = "",
                IncludeIgnoredPlates = true, // Include ignored plates
                PageNumber = 0,
                PageSize = 10
            };
            ...
            // Assert
            result.Should().NotBeNull();
            result.Plates.Should().HaveCount(2); // Should include both normal and ignored plates
            
            var normalTestPlate = result.Plates.Single(p => p.PlateNumber == "NORMAL123");
            normalTestPlate.IsIgnore.Should().BeFalse();
        ...
        public async Task Handle_WithAlertsAndIgnores_MarksPlatesToAlert()
        {
            ...
            {
                PlateNumber = "",
                IncludeIgnoredPlates = false, // Exclude ignored plates
                PageNumber = 0,
                PageSize = 10
            };
            ...
            // Assert
            result.Should().NotBeNull();
            result.Plates.Should().HaveCount(2); // Should exclude the ignored plate

            var alertedPlate = result.Plates.Single(p => p.PlateNumber == "ALERT123");
            alertedPlate.IsAlert.Should().BeTrue();

            var normalTestPlate = result.Plates.Single(p => p.PlateNumber == "NORMAL123");
            normalTestPlate.IsAlert.Should().BeFalse();
            normalTestPlate.IsIgnore.Should().BeFalse();

            // Ignored plate should not be in results
            result.Plates.Should().NotContain(p => p.PlateNumber == "IGNORED123");
        }
