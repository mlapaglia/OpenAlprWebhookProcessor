...
namespace Tests.WebhookProcessor
{
    ...
    public class GroupWebhookHandlerTests : TestBase
    {
        ...
        public async Task HandleWebhookAsync_WithNonBulkImportAndOverlayEnabled_SchedulesOverlayUpdate()
        {
            ...
            // Assert
            await _cameraUpdateService.Received().ScheduleOverlayAsync(Arg.Any<CameraUpdateRequest>());
        }
        ...
        [Test]
        public async Task HandleWebhookAsync_WithWebhookForwardsAndPreview_ForwardsPreviewWebhook()
        {
            // Arrange
            var agent = TestDataFactory.CreateTestAgent();
            await UnitOfWork.Agents.AddAsync(agent);
            ...
            var webhookForward = TestDataFactory.CreateTestWebhookForward("https://localhost/webhook");
            webhookForward.ForwardGroups = false;
            webhookForward.ForwardGroupPreviews = true;
            await UnitOfWork.WebhookForwards.AddAsync(webhookForward);
            
            await UnitOfWork.SaveChangesAsync();

            var webhook = CreateTestWebhook(cameraId: 1, isParked: false, plateNumber: "PREVIEW123", isPreview: true);
        ...
        [Test]
        public async Task HandleWebhookAsync_WithWebhookForwardsDisabled_DoesNotForwardWebhook()
        {
            // Arrange
            var agent = TestDataFactory.CreateTestAgent();
            await UnitOfWork.Agents.AddAsync(agent);
            ...
            var webhookForward = TestDataFactory.CreateTestWebhookForward("https://localhost/webhook");
            webhookForward.ForwardGroups = false;
            webhookForward.ForwardGroupPreviews = false;
            await UnitOfWork.WebhookForwards.AddAsync(webhookForward);
            
            await UnitOfWork.SaveChangesAsync();

            var webhook = CreateTestWebhook(cameraId: 1, isParked: false, plateNumber: "NOFORWARD123", isPreview: false);
