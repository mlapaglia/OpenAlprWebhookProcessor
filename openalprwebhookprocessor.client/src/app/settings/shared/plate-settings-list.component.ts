              ...
              <ng-container matColumnDef="actions" i18n-matColumnDef>
                ...
                <td mat-cell *matCellDef="let element" class="actions-cell">
                  <app-actions-cell
                    [setting]="element"
                    [isEditing]="isEditing(element)"
                    (startEdit)="startEdit.emit(element)"
                    (saveEdit)="saveEdit.emit(element)"
                    (cancelEdit)="cancelEdit.emit()"
                    (confirmDelete)="confirmDelete.emit(element)" />
                </td>
              </ng-container>
...
export class PlateSettingsListComponent<T extends IPlateSetting> {
  readonly settings = input(new MatTableDataSource<T>([]));
  readonly config = input.required<ListConfig>();
  ...
  readonly editingSettingChange = output<T>();
