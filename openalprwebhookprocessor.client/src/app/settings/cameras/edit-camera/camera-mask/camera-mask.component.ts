import { type ElementRef, type OnInit, type OnDestroy, Component, inject, ChangeDetectionStrategy, input, viewChild } from '@angular/core';
...
export class CameraMaskComponent extends OnPushBaseComponent implements OnInit, OnDestroy {
  ...
  readonly savingCanvas = viewChild.required<ElementRef<HTMLCanvasElement>>('savingCanvas');
  readonly measureDiv = viewChild.required<ElementRef<HTMLDivElement>>('measureDiv');
  ...
  private loadImageIntoCanvas(url: string) {
    ...
    this.subscribeAndMarkForCheck(
      this.cameraMaskService.getPlateCapture(url),
      (image: Blob) => {
  ...
  private loadMaskCoordinates() {
    ...
    if (this.coordinates.length === 0) {
      this.subscribeAndMarkForCheck(
        this.cameraMaskService.getCameraMaskCoordinates(this.camera().id),
        (coordinates) => {
  ...
  public saveMask() {
    ...
    this.subscribeAndMarkForCheck(
      this.cameraMaskService.upsertImageMask(cameraMask),
      () => {
  ...
  private draw() {
    ...
    if (this.coordinates.length > 0) {
      ...
      this.coordinates.forEach((point, index) => {
        ...
        if (index === this.coordinates.length - 1 && !this.isClosed) {
  ...
  private getSamplePlates() {
    this.subscribeAndMarkForCheck(
      this.cameraMaskService.getPlateCaptures(this.camera().id),
      (plates) => {
