...
namespace OpenAlprWebhookProcessor.Features.Webhooks.WebhookProcessor.OpenAlprWebsocket
{
    public class WebsocketClientOrganizer : IWebsocketClientOrganizer
    {
        ...
        public WebsocketClientOrganizer(
            IOptions<WebsocketClientOrganizerConfiguration> configuration,
            ILogger<WebsocketClientOrganizer> logger)
        {
            _responseTimeout = TimeSpan.FromMilliseconds(configuration.Value.TimeoutMilliseconds);
        ...
        public async Task DisconnectAllClientsAsync(CancellationToken cancellationToken = default)
        {
            ...
            {
                var task = DisconnectClientSafelyAsync
                    (kvp.Key,
                    kvp.Value,
                    cancellationToken);
