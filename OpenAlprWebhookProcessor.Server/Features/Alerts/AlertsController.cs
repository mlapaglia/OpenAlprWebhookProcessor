...
namespace OpenAlprWebhookProcessor.Features.Alerts
{
    ...
    public class AlertsController : ControllerBase
    {
        ...
        [HttpPut("{id}")]
        public async Task<ActionResult> UpdateAlert(Guid id, [FromBody] Alert alert, CancellationToken cancellationToken)
        {
            if (id != alert.Id)
            {
                return BadRequest("Alert ID in URL does not match alert ID in body");
            }

            var command = new UpdateAlertCommand(alert);
            await _mediator.Send(command, cancellationToken);
            return Ok();
        }

        [HttpDelete("{id}")]
        public async Task<ActionResult> DeleteAlert(Guid id, CancellationToken cancellationToken)
        {
