    ...
    public class LicensePlatePredictionService : ILicensePlatePredictionService
    {
            ...
            CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                if (model == null)
                {
                    ...
                    return await CreateFallbackPredictionAsync(input, cancellationToken);
                }
                ...
                // Get agent timezone offset for proper time calculation
                var agent = await GetAgentAsync(cancellationToken);
                var timezoneOffset = agent?.TimeZoneOffset ?? 0.0;
                var currentTimeInTimezone = DateTimeOffset.UtcNow.AddHours(timezoneOffset);
                prediction.PredictedNextSeen = currentTimeInTimezone.AddHours(prediction.PredictedHours).DateTime;
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                if (model == null)
                {
                    ...
                    var fallbackTasks = inputs.Select(input => CreateFallbackPredictionAsync(input, cancellationToken));
                    return (await Task.WhenAll(fallbackTasks)).ToList();
                }

                var predictionEngine = _mlContext.Model.CreatePredictionEngine<LicensePlateTrainingData, LicensePlatePrediction>(model);
                
                // Get agent timezone offset once for all predictions
                var agent = await GetAgentAsync(cancellationToken);
                var timezoneOffset = agent?.TimeZoneOffset ?? 0.0;
                var currentTimeInTimezone = DateTimeOffset.UtcNow.AddHours(timezoneOffset);
                ...
                foreach (var input in inputs)
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        results.Add(await CreateFallbackPredictionAsync(input, cancellationToken));
                    }
                }
            ...
            CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                // Get agent timezone offset for proper time calculations
                var agent = await GetAgentAsync(cancellationToken);
                var timezoneOffset = agent?.TimeZoneOffset ?? 0.0;
                var currentTimeInTimezone = DateTimeOffset.UtcNow.AddHours(timezoneOffset);
                ...
                foreach (var plate in recentPlates)
                {
                    try
                    {
                        ...
                        if (prediction.PredictedNextSeen <= currentTimeInTimezone.AddHours(cutoffHours).DateTime)
                        {
        ...
        private async Task<LicensePlatePredictionResult> CreateFallbackPredictionAsync(LicensePlateInput input, CancellationToken cancellationToken)
        {
            ...
            var timeSinceLastSeen = currentTimeInTimezone.DateTime - input.LastSeen;
            ...
            {
                LicensePlate = input.LicensePlate,
                PredictedNextSeen = currentTimeInTimezone.AddHours(fallbackHours).DateTime,
        ...
        private async Task<Data.Agent> GetAgentAsync(CancellationToken cancellationToken)
        {
            using var scope = _serviceProvider.CreateScope();
            var unitOfWork = scope.ServiceProvider.GetRequiredService<IUnitOfWork>();
            return await unitOfWork.Agents.GetFirstAgentAsync(cancellationToken);
        }
