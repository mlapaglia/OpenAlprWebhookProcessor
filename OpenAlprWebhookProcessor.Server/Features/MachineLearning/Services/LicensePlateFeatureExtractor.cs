...
namespace OpenAlprWebhookProcessor.Features.MachineLearning.Services
{
    ...
    public class LicensePlateFeatureExtractor : ILicensePlateFeatureExtractor
    {
        ...
        private float CalculateTimeSinceLastSeen(List<PlateGroup> sightings, PlateGroup current)
        {
        ...
        private float CalculateHistoricalFrequency(List<PlateGroup> sightings)
        {
            if (sightings.Count <= 1) return 0;
        ...
        private float CalculateAverageTimeBetweenVisits(List<PlateGroup> sightings)
        {
            if (sightings.Count <= 1) return 0;
        ...
        private float CalculateSeasonalFactor(DateTime dateTime)
        {
        ...
        private float EncodeVehicleType(string vehicleType)
        {
        ...
        private float EncodeVehicleColor(string vehicleColor)
        {
    ...
    {
        Task<List<LicensePlateTrainingData>> ExtractTrainingDataAsync(int batchSize = 10000, CancellationToken cancellationToken = default);
        Task<LicensePlateTrainingData> ExtractFeaturesForPredictionAsync(LicensePlateInput input, CancellationToken cancellationToken = default);
    }
} 